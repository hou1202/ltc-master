function setWebupload(uploadId, limit, uploadText, WebUploader, removeId) {
    ...
    uploader = WebUploader.create({
        pick: {
            id: uploadId + " .filePicker",
            label: '点击选择文件',
            multiple: true
        },
        dnd: uploadId + " .queueList",
        paste: document.body,
        ...
        fileNumLimit: limit,
        fileSizeLimit: limit * 1024 * 1024,    // 6 M
        fileSingleSizeLimit: 1024 * 1024    // 1 M
    });

    // 添加“添加文件”的按钮，
    uploader.addButton({
        id: uploadId + " .filePicker2",
        label: '继续添加'
    });
    ...
    function updateStatus() {
        ...
        if (state === 'ready') {
            ...
            if (fileCount == stats.successNum && stats.successNum != 0) {
                $(uploadId + ' .webuploader-element-invisible').remove();
            }
        }
    ...
    uploader.onUploadSuccess = function (file, response) {
        var imageStr = $(uploadText).val();
        if (firstUpload) {
            firstUpload = false;
            imageStr = response.data;
    ...
    function setState(val) {
        ...
        switch (state) {
            ...
            case 'ready':
                ...
                $(uploadId + " .filePicker2").removeClass('element-invisible');
            ...
            case 'uploading':
                $(uploadId + " .filePicker2").addClass('element-invisible');
                $progress.show();
                $upload.text('暂停上传');
                break;
